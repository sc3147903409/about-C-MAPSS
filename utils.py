...
def get_data(dataset, sensors, sequence_length, alpha, threshold):
    # files
    dir_path = 'E:\study\复现资料\RUL-RVE\SIMPAC-2022-58-main\data/'
    ...
    y_test['RemainingUsefulLife'] = y_test['RemainingUsefulLife'].apply(lambda x: fun(x))
    y_test = y_test.RemainingUsefulLife.values
    ...
    X_test_pre = add_operating_condition(test)
...
def get_callbacks(model, data, target):
    model_callbacks = [
        ...
        ModelCheckpoint(filepath='E:\study\复现资料\RUL-RVE\SIMPAC-2022-58-main/checkpoints/checkpoint', monitor='val_loss', mode='min', verbose=1,
                        save_best_only=True, save_weights_only=True),
        TensorBoard(log_dir='E:\study\复现资料\RVE复现保存地址/logs'),
...
def viz_latent_space(encoder, data, targets=[], epoch='Final', save=False, show=True):
    ...
    if show:
        plt.show()
    if save:
        plt.savefig('E:\study\复现资料\RVE复现保存地址/images/latent_space_epoch' + str(epoch) + '.png')
    return z
